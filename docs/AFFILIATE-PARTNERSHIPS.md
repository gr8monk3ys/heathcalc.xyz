# Affiliate Partnership Targets (US)

This doc captures the highest-ROI **direct partnership** targets for meal delivery and app products. These typically outperform Amazon search links once approved, but require program acceptance to get real tracking links or discount codes.
...
1. Apply to the three priority programs above and request **trackable links + discount codes**.
2. Once accepted, swap the corresponding `url` values in `src/constants/affiliates.ts`.
3. If MyFitnessPal’s network listing is suitable, apply there; otherwise keep the Amazon/search links until a formal program exists.
