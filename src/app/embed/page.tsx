...
function CopyableEmbedCode({
  ...
  const embedUrl = `${SITE_URL}/api/embed/${calculator.slug}${theme === 'dark' ? '?theme=dark' : ''}`;
  const embedCode = `<iframe src="${embedUrl}" width="400" height="${calculator.defaultHeight}" frameborder="0" title="${calculator.title} - HealthCheck" style="border:none;border-radius:12px;max-width:100%;" loading="lazy"></iframe>`;
  ...
  return (
    <div>
      <textarea
        ...
        value={embedCode}
        rows={3}
        className="w-full p-3 neumorph-inset rounded-lg text-xs font-mono focus:outline-none focus:ring-2 focus:ring-accent resize-none mb-2"
        onClick={e => (e.target as HTMLTextAreaElement).select()}
        aria-label={`Embed code for ${calculator.title}`}
      />
      <button
        onClick={handleCopy}
        className="px-3 py-1.5 neumorph text-accent font-medium rounded-lg hover:shadow-neumorph-inset transition-all focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 text-xs"
...
export default function EmbedPage(): React.JSX.Element {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      {/* Hero Section */}
      <div className="mb-10">
        <h1 className="text-3xl font-bold mb-3">Embed Free Health Calculators</h1>
        ...
        <div className="flex flex-wrap gap-3 text-sm">
          <span className="px-3 py-1 neumorph rounded-full">Free to use</span>
          <span className="px-3 py-1 neumorph rounded-full">No registration required</span>
          <span className="px-3 py-1 neumorph rounded-full">Mobile responsive</span>
          <span className="px-3 py-1 neumorph rounded-full">Light and dark themes</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <select
          ...
          className="p-2 neumorph-inset rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-accent"
        >
      ...
      <div className="space-y-6 mb-10">
        {EMBEDDABLE_CALCULATORS.map(calc => (
          <div key={calc.slug} className="neumorph p-6 rounded-lg">
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
              <div className="flex-1">
                <h3 className="text-lg font-semibold mb-1">{calc.title}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{calc.description}</p>
              ...
              <button
                ...
                className="px-3 py-1.5 neumorph text-gray-600 dark:text-gray-400 font-medium rounded-lg hover:shadow-neumorph-inset transition-all focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 text-xs whitespace-nowrap self-start"
            ...
            {previewSlug === calc.slug && (
              <div className="mt-4 neumorph-inset rounded-lg p-3 overflow-auto">
                <p className="text-xs text-gray-500 dark:text-gray-500 mb-2">
                  Preview (scaled to fit)
                </p>
                <iframe
                  src={`${SITE_URL}/api/embed/${calc.slug}${globalTheme === 'dark' ? '?theme=dark' : ''}`}
                  width={400}
